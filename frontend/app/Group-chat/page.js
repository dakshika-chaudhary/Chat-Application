...
export default function GroupChatPage() {
  ...
  useEffect(() => {
    ...
    if (!storedGroup) {
      router.push('/select-group'); // Redirect if no group selected
      return;
    }
    ...
    socket.emit('joinGroup', storedGroup);
    ...
    <div>
      ...
      <div className="container mx-auto mt-5 max-w-screen-xl bg-gray-800 rounded-lg p-6 shadow-lg overflow-y-auto">
        ...
        <h1 className="text-3xl font-bold mb-3 text-center text-white -mt-3">
          {username} in <span className="text-yellow-400">{groupName}</span> Group
        </h1>
          ...
          <ul className="space-y-2">
            {messages.map((msg, index) => (
              ...
              >
                <div
                  className={`${
                    msg.sender === username ? 'bg-blue-500' : 'bg-gray-400'
